...
## This R script loads in the Feb 1 - Feb 2 2007 data and creates the fourth 
...
## Now we're ready to create the fourth required plot, which is actually a 2x2 
## "panel" of plots. 2 of the plots are basically repeats of the previous plots,
## two of the plots are new.

par(mfcol = c(2,2))

## Plot the top left graph first
with(powerData, plot(dateTime, Global_active_power, type="l", xlab = "", 
                     ylab = "Global Active Power (kilowatts)"))

## Now plot the bottom left graph (note that "mfcol" populates the graphs by column first)
...
legend("topright", names(powerData)[7:9], lty = 1, col = c("black", "red", "blue"), bty = "n") 


## Now plot the top right graph
with(powerData, plot(dateTime, Voltage, type="l", xlab = "datetime",
                     ylab = "Voltage"))


## Finally, plot the bottom right graph
